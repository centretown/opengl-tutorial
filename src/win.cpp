...
void mouse_callback(GLFWwindow *window, double xposIn, double yposIn)
{
    ...
    float yoffset =
        winState.lastY - ypos; // reversed since y-coordinates go from bottom to top
...
GLFWwindow *InitWindow(Camera *camera, int width, int height)
{
    ...
    GLFWwindow *window =
        glfwCreateWindow(width, height, "LearnOpenGL OpenGL ES 2.0",
                         NULL, NULL);
    if (!window)
    {
        glfwWindowHint(GLFW_CONTEXT_CREATION_API, GLFW_NATIVE_CONTEXT_API);
        window = glfwCreateWindow(width, height, "LearnOpenGL OpenGL ES 2.0 Native", NULL, NULL);
        if (!window)
        {
            glfwTerminate();
            exit(EXIT_FAILURE);
